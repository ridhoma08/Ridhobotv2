/*
Hallo iam RidhoMaulanaAbdi:)
SC By : RIDHOBOT MWAHH:*
*/
  ...
  for (i = 0; i < 3; i++) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ridhoNumber = ["628998839509@s.whatsapp.net"] // JANGAN DI UBAH NGENTOD
			...
			switch(command) {
               ...
               client.sendMessage(from, 'Nih nomor pacarku😍, ehh ownerku hehee😁 jangan di galakin ya:*',MessageType.text, { quoted: mek} )
				...
				case 'bc':
					if (!isOwner) return reply('Kamu siapa?')
					if (!isRidho) return reply('Kamu Ridho?')
